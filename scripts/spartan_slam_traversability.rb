...
Orocos::Process.run(
   ...
   'gyro',
   'unit_vicon',
   'unit_shutter_controller',
   'unit_hazard_detector',
   'fdir',
   'temperature',
...
do
    ...
    path_planner.keep_old_waypoints = false
    if options[:v] == false
    ...
    vicon.pose_samples.connect_to                       viso2_evaluation.groudntruth_pose_not_aligned

    #pancam_panorama.tilt_angle_out_degrees.connect_to   pancam_transformer.pitch
    #pancam_panorama.pan_angle_out_degrees.connect_to    pancam_transformer.yaw
...
#        vicon.pose_samples.connect_to                       ga_slam.orbiterCloudPose
        ...
        viso2_evaluation.pose_samples_out.connect_to          waypoint_navigation.pose
        viso2_evaluation.pose_samples_out.connect_to          path_planner.pose
        viso2_evaluation.pose_samples_out.connect_to          traversability.pose
        viso2_evaluation.pose_samples_out.connect_to          ga_slam.odometryPose
        viso2_evaluation.pose_samples_out.connect_to          orbiter_preprocessing.robotPose
        viso2_evaluation.pose_samples_out.connect_to          ga_slam.orbiterCloudPose

        puts "using vicon and SpartanVO"
    end
    ...
    if options[:v] == false
      # To the starting position
        ...
        goal.heading = 135.0*3.141592/180.0
      ...
      # To the desired destination
        goal.position[0] = 45.0
        goal.position[1] = 85.0
      ...
      # First waypoint to test
        goal.position[0] = 5.00
        goal.position[1] = 5.00
        goal.position[2] = 0.00
        goal.heading = 95.00*3.141592/180.0
        goal_writer.write(goal)

        #random_travmap_writer = path_planner.set_random_travmap.writer
        #random_travmap_writer.write(true)
